...
export async function importCharacter(file:File):Promise<Character>{
 ...
 const extensions=(data as unknown as {extensions?:Record<string,unknown>}).extensions??{};character.compatibilityWarnings=[];if(Object.keys(extensions).some(k=>k!=='opencharacter'))character.compatibilityWarnings.push('extensions');if(/{{(?!char}}|user}})[^}]+}}/.test([data.description,data.personality,data.scenario,data.system_prompt,data.post_history_instructions,data.mes_example].join(' ')))character.compatibilityWarnings.push('macros');
