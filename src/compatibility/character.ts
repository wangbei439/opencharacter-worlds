...
export function normalizeBook(raw:Record<string,unknown>,characterId?:string):Worldbook{
 const entries=(Array.isArray(raw.entries)?raw.entries:Object.values((raw.entries??{}) as Record<string,unknown>)).slice(0,10000).map((item,index)=>{const e=item as Record<string,unknown>;return {id:String(e.id??index),keys:stringArray(e.keys??e.key),secondaryKeys:stringArray(e.secondary_keys??e.keysecondary),content:asString(e.content),enabled:e.enabled!==false&&e.disable!==true,constant:e.constant===true,selective:e.selective===true,caseSensitive:e.case_sensitive===true,priority:Number(e.priority??e.insertion_order??e.order??0),position:e.position==='after_char'?'after_char':'before_char'} as LoreEntry});
 return {id:newId(),characterId,name:asString(raw.name)||'World Book',entries,raw:JSON.stringify(raw),scanDepth:Math.max(1,Math.min(100,Number(raw.scan_depth)||4)),tokenBudget:Math.max(128,Math.min(8192,Number(raw.token_budget)||1024))};
}
export async function importCharacter(file:File):Promise<Character>{
 ...
 const book=data.character_book?normalizeBook(data.character_book as unknown as Record<string,unknown>,id):undefined;if(book)character.worldbookIds.push(book.id);
...
export async function importWorldbook(file:File,characterId:string){
 ...
 await db.transaction('rw',books,characters,async()=>{await books.put(book);const character=await characters.get(characterId);if(character)await characters.update(characterId,{worldbookIds:[...character.worldbookIds,book.id]})});return book;
}
