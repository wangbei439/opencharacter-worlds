...
export interface Asset { id:string; characterId?:string; kind:'original'|'portrait'|'expression'|'background'|'audio'; name:string; mime:string; blob:Blob; sha256:string; expression?:Expression; }
export interface SceneDesign {backgroundIds:string[];musicIds:string[];rotate:boolean;rotationSeconds:number;musicVolume:number;overlayOpacity:number;portraitScale:number;particles:Particle;}
export const defaultDesign:SceneDesign={backgroundIds:[],musicIds:[],rotate:false,rotationSeconds:20,musicVolume:0.35,overlayOpacity:0.4,portraitScale:1,particles:'dust'};
export interface Character { id:string; name:string; description:string; personality:string; scenario:string; examples:string; systemPrompt:string; postHistory:string; greetings:string[]; creator:string; tags:string[]; originalAssetId:string; portraitId?:string; expressions:Partial<Record<Expression,string>>; worldbookIds:string[]; createdAt:number; sourceSpec:string; builtin?:boolean; worldview?:string; design?:SceneDesign; }
...
export interface ProviderConfig { id:string; kind:'openai'|'claude'|'deepseek'|'glm'|'qwen'|'custom'|'mock'|'openrouter'|'compatible'|'xai'|'gemini'; protocol?:'openai'|'anthropic'; baseUrl:string; model:string; apiKey:string; headers:Record<string,string>; contextLimit:number; maxTokens:number; temperature:number; topP:number; streaming:boolean; structuredOutput:boolean; }
export const newId=()=>crypto.randomUUID();
export const defaultProvider:ProviderConfig={id:'primary',kind:'openai',protocol:'openai',baseUrl:'https://api.openai.com/v1',model:'',apiKey:'',headers:{},contextLimit:16384,maxTokens:1024,temperature:0.8,topP:1,streaming:true,structuredOutput:false};
