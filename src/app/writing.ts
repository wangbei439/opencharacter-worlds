...
import {writingSchema,presetFileSchema,type WritingSettings} from '../domain/writing.ts';
...
export async function loadWritingPresets():Promise<WritingPreset[]>{const value=(await table.get('writing-presets'))?.value;return Array.isArray(value)?value.filter(p=>typeof p.id==='string'&&typeof p.name==='string'&&writingSchema.safeParse(p.writing).success):[]}
...
export async function importWritingPreset(file:File){if(file.size>256*1024)throw Error('fileTooLarge');let parsed;try{parsed=presetFileSchema.parse(JSON.parse(await file.text()))}catch{throw Error('invalidPreset')}return storeWritingPreset(parsed.name,parsed.writing)}
