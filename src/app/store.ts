...
interface AppState {ready:boolean;speakingCharacterId?:string;updateReady?:boolean;route:'landing'|'wizard'|'play';wizardStep:number;selectedCharacterId?:string;activeChatId?:string;characters:Character[];chats:Chat[];messages:Message[];world?:WorldState;events:WorldEvent[];transactions:Transaction[];books:Worldbook[];personas:Persona[];provider:ProviderConfig;settings:Settings;sending:boolean;streamText:string;expression:Expression;context?:ContextReport;error?:{code:string;detail?:string};notice?:string;}
...
export function reportError(e:unknown){const error=e as {code?:string;message?:string;detail?:string;name?:string};if(error?.name==='QuotaExceededError'){patchApp({error:{code:'storageFull'}});return}if(error?.name==='AbortError')return;patchApp({error:{code:error.code??error.message??'unknown',detail:error.detail}})}
