...
async function generate(chat:Chat,character:Character,user:Message,action?:Partial<Candidate>,replace?:Message,continuation=false,groupTurn=false){
 ...
 try{
  ...
  if(chat.writing?.vectorsEnabled){try{const {recallVectors}=await import('./retrieval.ts');const recalled=await recallVectors(config,chat.writing.embeddingModel,ledger,character.id,user.content,signal);if(recalled.length)extras.push({name:'vectorMemory',content:'COMMITTED_EVENTS (semantic recall)\n'+recalled.map(r=>r.id+' ['+r.score.toFixed(3)+'] '+r.content).join('\n')})}catch(e){if(signal.aborted)throw e;patchApp({notice:'vectorFallback'})}}
