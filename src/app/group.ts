...
export async function configureGroup(rounds?:number,order?:'ordered'|'shuffle'){if((rounds!==undefined&&(!Number.isInteger(rounds)||rounds<1||rounds>3))||(order!==undefined&&!['ordered','shuffle'].includes(order)))throw Error('invalidGroupSchedule');const s=useApp.getState();if(s.sending||!s.activeChatId)return;await chats.update(s.activeChatId,{...(rounds!==undefined?{groupRounds:rounds}:{}),...(order!==undefined?{groupOrder:order}:{})});await refresh(s.activeChatId)}
