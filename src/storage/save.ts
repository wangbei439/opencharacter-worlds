...
export async function exportSave(chatId:string,settings:Settings):Promise<Blob>{
 ...
 const assetIds=new Set([character.originalAssetId,character.portraitId,...Object.values(character.expressions),...[world,...history.flatMap(t=>t.before?[t.before]:[])].flatMap(w=>Object.values(w.entities).map(e=>e.backgroundId))].filter((id):id is string=>!!id));
 ...
 const payload={format:'ocwsave',version:1,createdAt:Date.now(),character,chat,world,messages:await messages.where('chatId').equals(chatId).sortBy('createdAt'),events:await events.where('worldId').equals(chatId).toArray(),transactions:history,worldbooks:await books.where('characterId').equals(character.id).toArray(),assets:serialized,...(chat.personaId?{persona:await personas.get(chat.personaId)}:{}),settings};
