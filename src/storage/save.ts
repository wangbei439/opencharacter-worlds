...
export async function exportSave(chatId:string,settings:Settings):Promise<Blob>{
 ...
 const allMessages=await messages.where('chatId').equals(chatId).sortBy('createdAt');const memberIds=[...new Set([...(chat.memberIds??[]),...allMessages.flatMap(m=>m.speakerId?[m.speakerId]:[])])].filter(id=>id!==character.id);const additionalCharacters=(await characters.bulkGet(memberIds)).filter(c=>!!c);const roster=[character,...additionalCharacters];
 const assetIds=new Set([...roster.flatMap(c=>[c.originalAssetId,c.portraitId,...Object.values(c.expressions),...(c.design?.backgroundIds??[]),...(c.design?.musicIds??[])]),...[world,...history.flatMap(t=>t.before?[t.before]:[])].flatMap(w=>Object.values(w.entities).map(e=>e.backgroundId))].filter((id):id is string=>!!id));
 ...
 const payload={format:'ocwsave',version:1,createdAt:Date.now(),character,additionalCharacters,chat,world,messages:await messages.where('chatId').equals(chatId).sortBy('createdAt'),events:await events.where('worldId').equals(chatId).toArray(),transactions:history,worldbooks:(await books.bulkGet([...new Set(roster.flatMap(c=>c.worldbookIds))])).filter(b=>!!b),assets:serialized,...(chat.personaId?{persona:await personas.get(chat.personaId)}:{}),settings};
...
export async function importSave(file:File){
 ...
 for(const row of [...(save.additionalCharacters??[]),...save.messages,...save.events,...save.transactions,...save.assets,...save.worldbooks,...(save.persona?[save.persona]:[])])ids.set(row.id,newId());
 const remapCharacter=(c:typeof save.character)=>({...c,id:ids.get(c.id)!,design:c.design?{...c.design,backgroundIds:c.design.backgroundIds.map(id=>ids.get(id)!),musicIds:c.design.musicIds.map(id=>ids.get(id)!)}:undefined,originalAssetId:ids.get(c.originalAssetId)!,portraitId:c.portraitId?ids.get(c.portraitId):undefined,expressions:Object.fromEntries(Object.entries(c.expressions).map(([k,v])=>[k,ids.get(v)!])),worldbookIds:c.worldbookIds.map(id=>ids.get(id)!)});
 const character=remapCharacter(save.character);
