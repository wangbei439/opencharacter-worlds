...
export function remapWorld(world:WorldState,ids:Map<string,string>):WorldState{
 ...
 entities:Object.fromEntries(Object.entries(world.entities).map(([key,e])=>[ref(key),{...e,id:ref(e.id),owner:optional(e.owner),location:optional(e.location),keyId:optional(e.keyId),backgroundId:optional(e.backgroundId),knownBy:e.knownBy?.map(ref)}])),
