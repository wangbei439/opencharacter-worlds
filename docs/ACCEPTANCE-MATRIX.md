...
此文件是当前进度的主入口，取代按轮次累加的状态结论；旧文档保留作为历史证据。代码基线：`c0d8b25`（GitHub 已公开并通过首次 CI）。本次是审计与测试准备，没有运行新的真实 API 对话。
...
更新：新增 `/?acceptance` 准备入口，使用正式导入、独立成员群聊和存档流程；四人受控接口检查通过，真实模型质量仍待验收。详见 [方案与证据](MULTICARD-ACCEPTANCE.md)。
...
- **测试器差异须先处理**：旧 `?live-check` 只做三卡9轮、内存隔离；大世界为单叙事者扮演多个NPC，不等同独立成员群聊。`benchmark.mjs` 仍使用旧直接判定、裸JSON解析和64-token Resolver，不能直接宣称代表现有正式生成流程。
...
| U03 | 多卡和大世界验证 | 待验证 | [docs/LIVE-REVIEW-2026-09-10.md](../docs/LIVE-REVIEW-2026-09-10.md) — 旧三卡9轮报告有实际API证据与已发现问题；不是当前多角色/检索新版本20–100轮验收。 |
